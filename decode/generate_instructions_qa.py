...
def main():
    ...
    print("\n=== DEBUG - DETAILED VERIFICATION ===")
    encrypted_array, random_nums = create_array_for_claude(message)
    results = [a ^ r for a, r in zip(encrypted_array.flatten()[:len(message)], random_nums)]
    decoded = ''.join(chr(x) for x in results)
    
    print(f"Original message: '{message}'")
    print(f"XOR results (raw numbers): {results}")
    print(f"Decoded instruction: '{decoded}'")
    print("\nXOR results breakdown:")
    for i, r in enumerate(results):
        print(f"  Position {i}: {r} (ASCII: '{chr(r)}') XOR of {encrypted_array.flatten()[i]} ^ {random_nums[i]}")
    print("\nFull array for reference:")
